...
import com.unmsm.clinica.validacion.NumeroDocumento;
	...
	public boolean isValid(Object dto, ConstraintValidatorContext context) {
		try {
			...
			if (existe && !existePersona) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NoExiste.Persona.numeroDocumento}",
						this.campoNumeroDocumento, context);
				return false;
			}
			if (!existe && existePersona) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Existe.Persona.numeroDocumento}",
						this.campoNumeroDocumento, context);
				return false;
			}
		} catch (Exception e) {
