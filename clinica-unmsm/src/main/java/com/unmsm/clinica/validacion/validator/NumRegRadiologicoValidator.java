		...
		try {
			...
			if (numeroRegistro == -1) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty(
						"{NotNull.ExamenMedicoRadiologico.numeroRegistro}", this.campoNumeroRegistro, context);
				return false;
			}
			if (numeroRegistro < this.minNumeroRegistro) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Min.ExamenMedicoRadiologico.numeroRegistro}",
						this.campoNumeroRegistro, context);
				return false;
			}
			if (anio == null || anio.equals("null")) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NotNull.ExamenMedicoRadiologico.anio}",
						this.campoAnio, context);
				return false;
			}
			if (anio.trim().isEmpty()) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NotBlank.ExamenMedicoRadiologico.anio}",
						this.campoAnio, context);
				return false;
			}
			if (anio.length() != this.tamanioAnio) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Length.ExamenMedicoRadiologico.anio}",
						this.campoAnio, context);
				return false;
			}
			if (!anio.matches(Regex.SOLO_DIGITOS)) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Pattern.ExamenMedicoRadiologico.anio}",
						this.campoAnio, context);
				return false;
			}
			boolean existe = examenMedicoRadiologicoService.existeExamenMedicoRadiologico(numeroRegistro, anio);
			if (!existe) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty(
						"{NoExiste.ExamenMedicoRadiologico.numeroRegistro}", this.campoAnio, context);
				return false;
			}
			return true;
		} catch (Exception e) {
