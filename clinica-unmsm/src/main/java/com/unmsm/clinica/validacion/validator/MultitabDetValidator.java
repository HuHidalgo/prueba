...
public class MultitabDetValidator implements ConstraintValidator<MultitabDet, Object> {
	...
	public boolean isValid(Object oIdItem, ConstraintValidatorContext context) {
		...
		if (idItem == null || idItem.equals("null")) {
			ValidatorUtil.addCustomMessageWithTemplate("{NotNull.MultitabDet." + campoIdItem + "}", context);
			return false;
		}
		if (idItem.trim().isEmpty()) {
			ValidatorUtil.addCustomMessageWithTemplate("{NotBlank.MultitabDet." + campoIdItem + "}", context);
			return false;
		}
		if (idItem.length() < min || idItem.length() > max) {
			ValidatorUtil.addCustomMessageWithTemplate("{Length.MultitabDet." + campoIdItem + "}", context);
			return false;
		}
		boolean existeItem = !multiTabDetService.buscarPorIdTablaIdItem(idTabla, idItem).isEmpty();
		if (existe && !existeItem) {
			ValidatorUtil.addCustomMessageWithTemplate("{NoExiste.MultitabDet." + campoIdItem + "}", context);
			return false;
		}
		if (!existe && existeItem) {
			ValidatorUtil.addCustomMessageWithTemplate("{Existe.MultitabDet." + campoIdItem + "}", context);
			return false;
		}
		return true;
	}
