	...
	public boolean isValid(Object dto, ConstraintValidatorContext context) {
		try {
			...
			if (existe && !existeAlumno) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NoExiste.Alumno.codigoAlumno}",
						this.campoCodigoAlumno, context);
				return false;
			}
			if (!existe && existeAlumno) {
				ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Existe.Alumno.codigoAlumno}",
						this.campoCodigoAlumno, context);
				return false;
			}
		} catch (Exception e) {
			ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Excepcion.DTO.propiedad}", this.campoCodigoAlumno,
					context);
			return false;
		}
		return true;
