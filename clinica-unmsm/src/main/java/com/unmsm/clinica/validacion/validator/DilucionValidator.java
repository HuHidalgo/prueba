...
import com.unmsm.clinica.validacion.Dilucion;
		...
		try {
			...
			if (idRPR != null && idRPR.equals(ConstantesGenerales.RPR_REACTIVO)) {
				if (dilucion == null || dilucion.equals("null")) {
					ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NotNull.MultitabDet.dilucion}",
							this.campoDilucion, context);
					return false;
				}
				if (dilucion.trim().isEmpty()) {
					ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NotBlank.MultitabDet.dilucion}",
							this.campoDilucion, context);
					return false;
				}
				if (dilucion.length() < minDilucion || dilucion.length() > maxDilucion) {
					ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Length.MultitabDet.dilucion}",
							this.campoDilucion, context);
					return false;
				}
				boolean existe = !multiTabDetService.buscarPorIdTablaIdItem(MultiTablaUtil.TABLA_DILUCION, dilucion)
						.isEmpty();
				if (!existe) {
					ValidatorUtil.addCustomMessageWithTemplateWithProperty("{NoExiste.MultitabDet.dilucion}",
							this.campoDilucion, context);
					return false;
				}
				return true;
			...
			ValidatorUtil.addCustomMessageWithTemplateWithProperty("{Excepcion.DTO.propiedad}", this.campoDilucion,
					context);
			e.printStackTrace();
			return false;
		}
