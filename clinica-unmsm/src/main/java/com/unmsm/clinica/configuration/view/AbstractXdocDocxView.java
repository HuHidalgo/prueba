package com.unmsm.clinica.configuration.view;
...
public abstract class AbstractXdocDocxView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		IXDocReport ixDocReport = XDocReportRegistry.getRegistry().loadReport(
				getClass().getClassLoader().getResourceAsStream(StringsUtils.concatenarCadena(
						ConstantesGenerales.RUTA_REPORTE_DOCX, templateDocx, MimeTypeUtil.EXTENSION_DOCX)),
				TemplateEngineKind.Freemarker);
		IContext context = ixDocReport.createContext();
		reporteParametros.entrySet().stream().forEach(map -> context.put(map.getKey(), map.getValue()));
		String nombreReporte = (String) model.get(ConstantesGenerales.PARAM_NOMBRE_REPORTE);
		response.setHeader("Content-Disposition",
				StringsUtils.concatenarCadena("attachment;filename=", nombreReporte, MimeTypeUtil.EXTENSION_DOCX));
		response.setContentType(getContentType());
		renderReport(ixDocReport, context, response);
	}

	protected abstract void buildDocxDocument(Map<String, Object> model, Map<String, Object> parametros,
			IXDocReport ixDocReport, IContext contextReport, HttpServletRequest request, HttpServletResponse response);

	protected void renderReport(IXDocReport ixDocReport, IContext contextReport, HttpServletResponse response)
