...
public interface IProcesoAutomaticoMapper extends IMantenibleMapper<ProcesoAutomatico> {
	@Select(value = { "{call MANT_PROCESO_AUTOMATICO ( " + "#{verbo, jdbcType = VARCHAR, mode = IN},"
			...
			+ "#{objeto.diario, jdbcType = BIT, mode = IN}," + "#{objeto.secuencia, jdbcType = INTEGER, mode = IN},"
			+ "#{objeto.fechaInicio, jdbcType = DATE, mode = IN}," + "#{objeto.fechaFin, jdbcType = DATE, mode = IN},"
