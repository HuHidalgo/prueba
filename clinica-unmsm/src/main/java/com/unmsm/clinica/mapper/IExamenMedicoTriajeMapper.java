...
public interface IExamenMedicoTriajeMapper extends IMantenibleMapper<ExamenMedicoTriaje> {
	@Select(value = { "{call MANT_EXAMEN_MEDICO_TRIAJE ( " + "#{verbo, jdbcType = VARCHAR, mode = IN},"
			...
			+ "#{objeto.tipoAlumno, jdbcType = VARCHAR, mode = IN}," + "#{objeto.peso, jdbcType = SMALLINT, mode = IN},"
			+ "#{objeto.talla, jdbcType = SMALLINT, mode = IN}," + "#{objeto.pulso, jdbcType = SMALLINT, mode = IN},"
			+ "#{objeto.presionSistolica, jdbcType = SMALLINT, mode = IN},"
			+ "#{objeto.presionDiastolica, jdbcType = SMALLINT, mode = IN},"
			+ "#{objeto.fum, jdbcType = DATE, mode = IN}," + "#{objeto.idCampania, jdbcType = INTEGER, mode = IN},"
