...
import com.unmsm.clinica.utilitario.CustomSpringExpressionLanguageParserUtil;
...
public class AuditableAspecto {
	...
	public Object registrarAuditoria(ProceedingJoinPoint proceedingJoinPoint, Audit audit) throws Throwable {
		boolean exito = true;
		Tipo tipo = null;
		String datos = "";
		...
		if (accion == Accion.Registro || accion == Accion.Actualizacion || accion == Accion.Eliminacion) {
			datos = this.obtenerDatos(audit.datos(), clazz, tieneClaseAnotacion);
			String datosParaAuditar = CustomSpringExpressionLanguageParserUtil
					.getDynamicValue(signature.getParameterNames(), proceedingJoinPoint.getArgs(), datos);
			comentarioParaAuditoria = String.format(comentario.toString(), tipo.toString(), datosParaAuditar);
		} else {
		...
		Auditoria auditoria = Auditoria.builder().codigoAuditoria(tipo.name().toUpperCase()).idAccion(accion.toString())
				.comentario(comentarioParaAuditoria).direccionIp(direccionIp).nombreUsuario(nombreUsuario).build();
		try {
	...
	public Comentario obtenerComentario(Comentario comentario, Class<?> clazz, boolean tieneClaseAnotacion) {
