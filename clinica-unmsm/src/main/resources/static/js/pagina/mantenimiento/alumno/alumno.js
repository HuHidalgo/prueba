$(document).ready(function() {
	...
	$formBuscarAlumno = $("#formBuscarAlumno");
	...
	$funcionUtil.crearSelect2($local.$tiposDocumento2, "Seleccione un Tipo de Documento");
	...
	$local.$buscar.on("click", function() {
		var criterioBusqueda = $formBuscarAlumno.serializeJSON();
		if ($funcionUtil.camposVacios($formBuscarAlumno)) {
			$funcionUtil.notificarException($variableUtil.camposVacios, "fa-exclamation-circle", "Información", "info");
			return;
		}
		if (criterioBusqueda.idTipoDocumento != "-1" && criterioBusqueda.numeroDocumento == ""){
			$funcionUtil.notificarException("Ingrese el número de documento", "fa-exclamation-circle", "Aviso", "danger");
			return;
		}
		if (criterioBusqueda.idTipoDocumento == "-1" && criterioBusqueda.numeroDocumento != ""){
			$funcionUtil.notificarException("Seleccione tipo de documento", "fa-exclamation-circle", "Aviso", "danger");
			return;
		}
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "mantenimiento/alumno?accion=buscarAlumno",
			...
			success : function(alumno) {
				...
				$local.tablaMantenimiento.rows.add(alumno).draw();
			},
			complete : function() {
