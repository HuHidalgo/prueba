$(document).ready(function() {
			...
			"data" : function(row) {
				if (row.resultado == null)
					...
					if (row.resultado == 'N')
						...
						return "<label class='label label-danger label-size-12'>OBSERVADO</label>";
			},
			"className" : "all filtrable dt-center",
			"title" : "Resultado"
		} ],
	...
	$local.$buscar.on("click", function() {
		...
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "reporte/atencion/diaria/laboratorio?accion=buscar",
	...
	$local.$exportar.on("click", function() {
		...
		window.location.href = $variableUtil.root + "reporte/atencion/diaria/laboratorio?accion=exportar&" + paramCriterioBusqueda;
	});
